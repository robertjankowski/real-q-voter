...
def plot_preference_sampling_experiment(with_weighted_opinion=False, q=None, filename=None, pdf=False):
    ...
    plot_experiment(with_preference_files, with_weighted_opinion, show_q=True, col='tab:blue')
    plot_experiment(without_preference_files, with_weighted_opinion, show_q=True, col='tab:orange')
    if filename:
        if pdf:
            plt.savefig(
